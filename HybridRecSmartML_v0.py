...
class HybridRecommendationSystem:
    ...
    def compute_combined_similarity(self):
        ...
        # Step 4: Combine collaborative and content-based similarities
        return
        idx = 0
        for i in tqdm(range(num_products), desc="Combining Similarities"):
            for j in range(i + 1, num_products):  # Only upper triangle
                content_similarity = self.similarity_matrix[i, j]
                collaborative_similarity = normalized_collaborative_similarities[idx]
                idx += 1

                # Combine collaborative similarity and content similarity using blend weights
                combined_similarity = (self.blend_weights['collaborative'] * collaborative_similarity +
                                    self.blend_weights['content'] * content_similarity)

                # Assign combined similarity symmetrically to the matrix
                self.combined_similarity_matrix[i, j] = combined_similarity
                self.combined_similarity_matrix[j, i] = combined_similarity  # Mirror for symmetry

        print("Combined similarity matrix computed successfully.")
    ...
    def recommend_new_or_non_interacted_products(self, user_id, n=5, verbose=False):
        ...
        # Create a dictionary for quick product lookup by product_id
        ...
        interacted_product_names = set(
            product_lookup[p_id]['product_name'].strip().lower() for p_id in interacted_product_ids if p_id in product_lookup
        )
        
        # Get all products as a list of product_id
        all_product_ids = self.products_df['product_id'].tolist()
        
        # Determine which products the user has not interacted with
        non_interacted_product_ids = set(all_product_ids) - interacted_product_ids

        # Filter out duplicated product names (case-insensitive)
        seen_product_names = set()
        filtered_non_interacted_products = []
        
        for product_id in non_interacted_product_ids:
            if product_id in product_lookup:
                product_name = product_lookup[product_id]['product_name'].strip().lower()
        ...
        # Precompute the indices of the interacted products in bulk for similarity calculations
        interacted_indices = [
            self.products_df.index[self.products_df['product_id'] == p_id].tolist()[0]
            for p_id in interacted_product_ids if not self.products_df[self.products_df['product_id'] == p_id].empty
        ]
        ...
        for product_id in filtered_non_interacted_products:
            if product_id in product_lookup:
                ...
                product_index_list = self.products_df.index[self.products_df['product_id'] == product_id].tolist()
                ...
                if product_id in interacted_product_ids or product_name.strip().lower() in interacted_product_names:
                    if verbose:
                ...
                # If product not found, count and skip it
                skipped_products_count += 1
                if verbose:
                    print(f"Product with ID '{product_id}' not found in products_df.")
                continue  # Skip this product if not found

            # Calculate similarity only with products the user has interacted with
            if interacted_indices:
                content_similarity = np.max(self.similarity_matrix[product_index, interacted_indices])
            else:
                content_similarity = 0  # If no interacted products, set similarity to 0
